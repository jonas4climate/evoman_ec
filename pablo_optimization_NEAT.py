...
ENEMIES = [1, 3, 4] # TODO: Change to enemy set desired. Script should be run again with other set
...
for (i, enemy) in enumerate(ENEMIES):
    ...
    def eval_genomes(genomes, config, run, stats_data, fitnesses_data, n_nodes_data, all_decisions, n_weights_data, pbar_gens):        
        ...
        for id, genome in genomes:
            """
            This loop will be run once per complexity
            """
        ...
        # TODO: Get the best fitness for the best genome of the current n_nodes complexity
        # TODO: Save it as checkpoint!
        # Similar to: p.add_reporter(neat.Checkpointer(generation_interval=GEN_INTERVAL_LOG, time_interval_seconds=None, filename_prefix=os.path.join(DATA_FOLDER, 'neat-checkpoint-')))
        # TODO: Load checkpoint as reference for next generation
        # Similar to: p = neat.Checkpointer.restore_checkpoint('neat-checkpoint-%i' % CHECKPOINT)
        # TODO: Increase CURRENT_COMPLEXITY counter
        # CURRENT_COMPLEXITY += 1
        # TODO: If we have checkpoints for each complexity and max(n_nodes) > MAXIMUM_COMPLEXITY -> BREAK
        # If LAST_CHECKPOINT_SAVED and CURRENT_COMPLEXITY == MAXIMUM_COMPLEXITY: return
